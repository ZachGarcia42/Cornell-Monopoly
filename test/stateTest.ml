...
let state_2 = init_state players [ 39; 37 ]
let state_3 = init_state players [ 3; 5 ]
let go = Go
let it = IncomeTax
let fp = FreeParking
let lt = LuxuryTax
...
let tests =
  [
    test "player list" players (init_state players [] |> player_list);
    ...
    test "cannot add same property" [ 39 ] (add_properties [] 39);
    test "can add different property" [ 24; 39 ] (add_properties [ 39 ] 24);
